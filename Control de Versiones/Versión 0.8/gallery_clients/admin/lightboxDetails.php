...
/*Recoge el id de la familia/tipo de trabajo seleccionado.*/
$idCliente = mysql_real_escape_string($_POST["idCliente"]);

if($idCliente != null || $idCliente != ""){
	global $con;
	/* Recorre todos los tipos de trabajos contratados en el cliente selecionado. */ 
	$sqlJobs ="SELECT `term_taxonomy_id` FROM `".table_prefix."term_relationships` WHERE `object_id` = ".$idCliente;
	$resultadoJobs = mysql_query($sqlJobs, $con);
	while ($rowJobs = mysql_fetch_assoc($resultadoJobs)) {
		$idJob = $rowJobs['term_taxonomy_id'];
		/* De toda la lista de trabajos, realizamos una busqueda de los que son familias y no los guardamos. */
		$sqlnoCategory ="SELECT `parent` FROM `".table_prefix."term_taxonomy` WHERE `term_taxonomy_id` = ".$idJob;
		$resultadonoCategory = mysql_query($sqlnoCategory, $con);
		
		$noCategory = mysql_result($resultadonoCategory, 0);
		if($noCategory != 0){
			/* Guardamos la información de todos los tipos de trabajos contratados por el cliente. */
			$sqlJob ="SELECT * FROM `".table_prefix."terms` WHERE `term_id` = ".$idJob;
