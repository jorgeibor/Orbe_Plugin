...
function listarFamilias(){
	...
	$isCategory = false;
	$sqlParent = "SELECT DISTINCT `".table_prefix."term_taxonomy`.`parent` FROM `".table_prefix."term_taxonomy` WHERE `taxonomy`='Tipo de trabajo' AND `parent`!=0";
	$resultadoParent = mysql_query($sqlParent, $con);
	while ($rowParent = mysql_fetch_assoc($resultadoParent)) {
		
		$idCategory = $rowParent['parent'];
	...
	if(empty($listFamily)){
		$msg[] = array (
			"isNull"=> true,
			"message"=> "No se ha encontrado familias"
		);
		echo json_encode($msg);
	}else{
		foreach($listFamily as $family => $info){
			$consulta = "SELECT `family_color` FROM `".table_prefix."config_plugin` WHERE `family_id`='".$info['id']."'";
	...
	/*
	$isCategory = false;
	$sqlParent = "SELECT `".table_prefix."term_taxonomy`.`parent` FROM `".table_prefix."term_taxonomy` WHERE `taxonomy`='Tipo de trabajo'";
	$resultadoParent = mysql_query($sqlParent, $con);
	while ($rowParent = mysql_fetch_assoc($resultadoParent)) {
...
function consultarColor(){
	...
	if($idFamiliaSelect != null || $idFamiliaSelect != ""){
		...
		if($colorCategory == null || $colorCategory == ""){
			$colorCategory="#757575";	
		}
