	...
	if($filtro != null || $filtro != ""){
		...
		if($isExist == NULL || $isExist == ""){
			...
			while ($rowPostSubCategory = mysql_fetch_assoc($resultIdSubCategory)) {
				...
				$sqlListClient ="SELECT Posts.`ID`, MAX(`".table_prefix."posts`.`ID`)AS IDImg, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente' AND `ID`=".$idCliente.")Posts ON `".table_prefix."posts`.`post_parent` = `Posts`.`ID`";
				...
				//echo "</br>";
				$idImg = $rowInfoClient['IDImg'];
				$sqlUrlImg = "SELECT `guid` FROM `".table_prefix."posts` WHERE `ID` = ".$idImg;
				//echo $sqlUrlImg;
				$resultadoUrlImg = mysql_query($sqlUrlImg, $con);
				$urlImg = mysql_result($resultadoUrlImg, 0);
		...
		//SELECT `Posts`.`ID`, `0rb3_posts`.`guid`, `0rb3_posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `0rb3_posts` INNER JOIN (SELECT `ID`, `post_title`, `post_content` FROM `0rb3_posts` WHERE `post_type`='cliente' ORDER BY `ID` DESC)Posts ON `0rb3_posts`.`post_parent` = `Posts`.`ID` GROUP BY `ID` ASC
		$sqlInfoClient = "SELECT Posts.`ID`, MAX(`".table_prefix."posts`.`ID`)AS IDImg, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `guid`,`post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente')Posts ON `".table_prefix."posts`.`post_parent` = Posts.`ID` GROUP BY Posts.`ID`";
		//$sqlInfoClient = "SELECT Posts.`ID`, `".table_prefix."posts`.`ID`, `".table_prefix."posts`.`guid`, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `guid`,`post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente')Posts ON `".table_prefix."posts`.`post_parent` = Posts.`ID` WHERE `".table_prefix."posts`.`ID` IN(SELECT MAX(`".table_prefix."posts`.`ID`) FROM `".table_prefix."posts` GROUP BY `ID`)";
		$resultadoInfoClient = mysql_query($sqlInfoClient, $con);
		//echo $sqlInfoClient."<br/>";
		/*PAGINACION*/
		...
		$total_paginas = ceil($num_total_registros / $CLIENTES_PAGINA);
			//MAX(`".table_prefix."posts`.`ID`)
			$sqlInfoClient = "SELECT Posts.`ID`, MAX(`".table_prefix."posts`.`ID`)AS IDImg, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `guid`,`post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente')Posts ON `".table_prefix."posts`.`post_parent` = Posts.`ID` GROUP BY Posts.`ID` LIMIT $inicio, $CLIENTES_PAGINA";
			//$sqlInfoClient = "SELECT Posts.`ID`, `".table_prefix."posts`.`ID`, `".table_prefix."posts`.`guid`, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `guid`,`post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente')Posts ON `".table_prefix."posts`.`post_parent` = Posts.`ID` WHERE `".table_prefix."posts`.`ID` IN(SELECT MAX(`".table_prefix."posts`.`ID`) FROM `".table_prefix."posts` GROUP BY `ID`) LIMIT $inicio, $CLIENTES_PAGINA";
			//echo $sqlInfoClient."<br/>";
		/************/
			$resultadoInfoClient = mysql_query($sqlInfoClient, $con);
