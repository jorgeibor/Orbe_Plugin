...
function listadoClientes() {
	...
	if($filtro != null || $filtro != ""){
		...
		$output = '';
			$output .= '<div id="divShortcode1" class="divShortcode1" >';
				$output .= '<div id="filterbox" class="filterbox">';
					...
					<div style="display: block; height: 150px;" id="filterForm" class="filterboxForm">
						<div class="filterSubDiv" style="float: left; margin: 0px 2.5%; width: 45%;">
							<span id="formNameFilterBox">
								Nombre: 
								<input id="nameClient" name="nameClient" type="text">
							</span>
							<span id="formOrderFilterBox">
								Ordenar:<br>
								<p style="margin-left: 20px; float: left;"> A-&gt;Z<input style="margin-left: 10px;" name="orderClientby" id="orderClientby" class="orderClientby" value="AZ" checked="" type="radio"></p>
								<p style="float: left; margin-left: 20px;">Z-&gt;A<input style="margin-left: 10px;" name="orderClientby" id="orderClientby" class="orderClientby" value="ZA" checked="" type="radio"></p>
								<br>
							</span>
						</div>
						<div class="filterSubDiv" style="float: left; margin: 0px 2.5%; width: 45%;">
							<span id="formFamilyFilterBox" class="formFamilyFilterBox">
								Categoria: 
								<select id="selectCategoria"></select>
							</span>
							<span id="formCategoryFilterBox" class="formCategoryFilterBox">
								Tipo de Trabajo: 
								<select id="selectTipoTrabajo"></select>
							</span>
						</div>
					</div>
					...
					$output .= '<div id="filterForm" class="filterboxForm">';
						$output .='<div class="filterSubDiv">';
							...
							$output .= '<span id="formOrderFilterBox">';
								...
								$output .= '<p style="float: right;"><input name="buscarCliente" id="buscarCliente" class="searchClient" value="Buscar" type="button"></p>';
							$output .= '</span>';
						$output .= '</div>';	
		...
		$sqlArrayClients = "SELECT `Posts`.`ID`, `".table_prefix."posts`.`guid`, `".table_prefix."posts`.`post_parent`, Posts.`post_title`, Posts.`post_content` FROM `".table_prefix."posts` INNER JOIN (SELECT `ID`, `post_title`, `post_content` FROM `".table_prefix."posts` WHERE `post_type`='cliente')Posts ON `".table_prefix."posts`.`post_parent` = `Posts`.`ID`";
			...
			$(function(){
				...
				$("#nameClient").autocomplete({
					...
					select: function(event, ui){
						...
						$.post("/orbevisualcom/wp-content/plugins/gallery_clients/admin/filters.php",{ idCategory : 'Nombre', nameClient : ui.item.label },function(cliente){
							console.log(cliente);
